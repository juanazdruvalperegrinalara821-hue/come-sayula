...
app.post('/api/restaurant/products',auth,restaurantAccess('can_manage_products'),(req,res)=>{
    ...
    if(image&&!image.startsWith('/uploads/'))return res.status(400).json({error:'Imagen inválida'});
    const result=db.prepare('INSERT INTO products(restaurant_id,name,description,price,image,category,available,stock_enabled,stock_quantity,low_stock_threshold) VALUES(?,?,?,?,?,?,1,?,?,?)')
        .run(restaurant.id,name,description,price,image,category,stockEnabled?1:0,stockQuantity,lowStockThreshold);
...
app.put('/api/restaurant/products/:id',auth,restaurantAccess('can_manage_products'),(req,res)=>{
    ...
    const result=db.prepare('UPDATE products SET name=?,description=?,price=?,image=?,category=?,stock_enabled=?,stock_quantity=?,low_stock_threshold=?,available=CASE WHEN ?=1 AND ?<=0 THEN 0 ELSE available END WHERE id=? AND restaurant_id=?')
        .run(name,description,price,image,category,stockEnabled?1:0,stockQuantity,lowStockThreshold,stockEnabled?1:0,stockQuantity,req.params.id,restaurant.id);
