...
const mercadoPagoConfigured=()=>Boolean(MERCADOPAGO_ACCESS_TOKEN&&['test','production'].includes(MERCADOPAGO_MODE));
...
app.patch('/api/restaurant/orders/:id',auth,restaurantAccess('can_manage_orders'),(req,res)=>{
    const restaurant=req.restaurant;
    const order=db.prepare('SELECT id,status,scheduled_for,estimated_prep_minutes,payment_status FROM orders WHERE id=? AND restaurant_id=?')
