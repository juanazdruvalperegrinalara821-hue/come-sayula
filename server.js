...
app.patch('/api/delivery/orders/:id',auth,role(['delivery']),(req,res)=>{
    ...
    if(nuevoEstado==='delivered'){
        ...
        let resultado;try{resultado=db.transaction(()=>{
            ...
            const trustActors=db.prepare('SELECT o.customer_id,r.owner_id FROM orders o JOIN restaurants r ON r.id=o.restaurant_id WHERE o.id=?').get(orderId);adjustTrust(trustActors.customer_id,1);adjustTrust(trustActors.owner_id,1);adjustTrust(req.user.id,1);rewardDeliveredOrder(orderId);
