...
const parseMenuDraft=(content,sourceType)=>{
    ...
    if(sourceType==='csv'&&lines.length){
        const header=parseCsvLine(lines[0]).map(value=>value.toLowerCase()),nameIndex=header.findIndex(value=>['name','nombre','producto'].includes(value)),priceIndex=header.findIndex(value=>['price','precio'].includes(value));
        if(nameIndex>=0&&priceIndex>=0)return lines.slice(1,301).map((line,index)=>{const cells=parseCsvLine(line),category=(cells[header.findIndex(value=>['category','categoria','categoría'].includes(value))]||'Comida').trim(),name=(cells[nameIndex]||'').trim(),price=Number(String(cells[priceIndex]||'').replace(/[$,\s]/g,'')),description=(cells[header.findIndex(value=>['description','descripcion','descripción'].includes(value))]||'').trim();return {name:name.slice(0,100),description:description.slice(0,500),price,category,error:menuDraftError(name,price,category),sortOrder:index};});
    }
    return lines.slice(0,300).map((line,index)=>{const parts=(sourceType==='csv'?parseCsvLine(line):line.split('|')).map(value=>value.trim()),priceAtEnd=line.match(/\$?\s*(\d+(?:\.\d{1,2})?)\s*$/),name=parts[0]||'',price=Number(String(parts[1]||priceAtEnd?.[1]||'').replace(/[$,\s]/g,'')),category=parts[2]||'Comida',description=parts[3]||'';return {name:name.replace(/\s*\$?\s*\d+(?:\.\d{1,2})?\s*$/,'').slice(0,100),description:description.slice(0,500),price,category,error:menuDraftError(name,price,category),sortOrder:index};});
};
...
app.get('/api/favorites',auth,role(['customer']),(req,res)=>res.json(db.prepare('SELECT r.id,r.name,r.category,r.image,r.operational_status FROM favorite_restaurants f JOIN restaurants r ON r.id=f.restaurant_id WHERE f.customer_id=? AND r.active=1 ORDER BY f.created_at DESC').all(req.user.id)));
app.put('/api/favorites/:restaurantId',auth,role(['customer']),(req,res)=>{const id=Number(req.params.restaurantId);if(!db.prepare('SELECT id FROM restaurants WHERE id=? AND active=1').get(id))return res.status(404).json({error:'Restaurante no encontrado'});db.prepare('INSERT OR IGNORE INTO favorite_restaurants(customer_id,restaurant_id) VALUES(?,?)').run(req.user.id,id);res.json({ok:true,favorite:true})});
...
app.get('/api/promotions',(req,res)=>res.json(db.prepare("SELECT p.id,p.title,p.description,p.expires_at,r.id restaurant_id,r.name restaurant_name FROM local_promotions p JOIN restaurants r ON r.id=p.restaurant_id WHERE p.active=1 AND r.active=1 AND (p.starts_at IS NULL OR datetime(p.starts_at)<=CURRENT_TIMESTAMP) AND (p.expires_at IS NULL OR datetime(p.expires_at)>CURRENT_TIMESTAMP) ORDER BY p.created_at DESC LIMIT 50").all()));
...
app.get('/api/restaurants/:id/business-hours',(req,res)=>{const restaurant=db.prepare('SELECT id FROM restaurants WHERE id=? AND active=1').get(Number(req.params.id));if(!restaurant)return res.status(404).json({error:'Restaurante no encontrado'});const hours=db.prepare('SELECT weekday,is_closed,opens_at,closes_at FROM restaurant_business_hours WHERE restaurant_id=? ORDER BY weekday').all(restaurant.id),status=restaurantScheduleStatus(restaurant.id,new Date());res.json({configured:hours.length===7,openNow:status.open,hours,timeZone:'America/Mexico_City'});});
...
app.post('/api/orders',auth,role(['customer']),rateLimit('orders',ORDER_RATE_LIMIT_MAX,10*60*1000),async(req,res,next)=>{try{
    ...
    const schedule=restaurantScheduleStatus(restaurant.id,timing.scheduledFor?new Date(timing.scheduledFor):new Date());if(!schedule.open)return res.status(409).json({error:timing.orderTiming==='scheduled'?'El restaurante no abre en la fecha y hora seleccionadas':'El restaurante está fuera de su horario de servicio'});
    ...
    const risk=assessOrderRisk(req.user.id,grossTotal,paymentMethod);
...
app.get('/api/restaurant/employees',auth,restaurantAccess(),restaurantOwner,(req,res)=>res.json(db.prepare(`SELECT u.id,u.name,u.email,u.phone,u.account_status,m.can_manage_orders,m.can_manage_products,m.can_view_finance,m.can_use_pos,m.active,m.created_at FROM restaurant_members m JOIN users u ON u.id=m.user_id WHERE m.restaurant_id=? ORDER BY m.created_at DESC`).all(req.restaurant.id)));
app.post('/api/restaurant/employees',auth,restaurantAccess(),restaurantOwner,rateLimit('restaurant-employees',12,60*60*1000),async(req,res)=>{const name=String(req.body.name||'').trim().slice(0,100),email=normalizeEmail(req.body.email),phone=String(req.body.phone||'').trim().slice(0,30),password=String(req.body.password||'');if(!name||!email||password.length<10)return res.status(400).json({error:'Completa nombre, correo y una contraseña de al menos 10 caracteres'});if(db.prepare('SELECT id FROM users WHERE email=?').get(email))return res.status(409).json({error:'Ese correo ya pertenece a otra cuenta'});const id=db.transaction(()=>{const created=db.prepare("INSERT INTO users(name,email,phone,password_hash,role,account_status,email_verified) VALUES(?,?,?,?, 'restaurant_employee','approved',0)").run(name,email,phone,bcrypt.hashSync(password,12));db.prepare('INSERT INTO restaurant_members(user_id,restaurant_id,can_manage_orders,can_manage_products,can_view_finance,can_use_pos) VALUES(?,?,?,?,?,?)').run(created.lastInsertRowid,req.restaurant.id,req.body.canManageOrders===false?0:1,req.body.canManageProducts?1:0,req.body.canViewFinance?1:0,req.body.canUsePos?1:0);return Number(created.lastInsertRowid)})();audit(req,'restaurant_employee_created','user',id);res.status(201).json({id});});
app.patch('/api/restaurant/employees/:id',auth,restaurantAccess(),restaurantOwner,(req,res)=>{const id=Number(req.params.id),active=req.body.active===false?0:1;const result=db.prepare('UPDATE restaurant_members SET can_manage_orders=?,can_manage_products=?,can_view_finance=?,can_use_pos=?,active=? WHERE user_id=? AND restaurant_id=?').run(req.body.canManageOrders?1:0,req.body.canManageProducts?1:0,req.body.canViewFinance?1:0,req.body.canUsePos?1:0,active,id,req.restaurant.id);if(result.changes!==1)return res.status(404).json({error:'Empleado no encontrado'});db.prepare("UPDATE users SET account_status=? WHERE id=? AND role='restaurant_employee'").run(active?'approved':'suspended',id);audit(req,'restaurant_employee_permissions_updated','user',id);res.json({ok:true});});
...
app.post('/api/restaurant/menu-drafts',auth,restaurantAccess('can_manage_products'),rateLimit('menu-draft-import',10,3600000),(req,res)=>{const type=String(req.body.sourceType||'text'),content=String(req.body.content||'');if(!['text','csv'].includes(type)||!content.trim()||Buffer.byteLength(content)>102400)return res.status(400).json({error:'Contenido inválido o mayor de 100 KB'});const items=parseMenuDraft(content,type);const id=db.transaction(()=>{const draft=Number(db.prepare('INSERT INTO menu_drafts(restaurant_id,source_type,source_name,created_by_user_id) VALUES(?,?,?,?)').run(req.restaurant.id,type,String(req.body.sourceName||'Texto pegado').slice(0,120),req.user.id).lastInsertRowid),insert=db.prepare('INSERT INTO menu_draft_items(draft_id,name,description,price,category,validation_error,sort_order) VALUES(?,?,?,?,?,?,?)');items.forEach(item=>insert.run(draft,item.name,item.description,Number.isFinite(item.price)?item.price:null,item.category,item.error,item.sortOrder));return draft;})();audit(req,'menu_draft_created','menu_draft',id);res.status(201).json({id,itemCount:items.length,errorCount:items.filter(i=>i.error).length});});
...
app.get('/api/group-orders/restaurants',auth,role(['customer']),(req,res)=>{refreshTemporaryAvailability();res.json(db.prepare("SELECT id,name FROM restaurants WHERE active=1 AND operational_status NOT IN ('closed','paused') ORDER BY name").all());});
app.post('/api/group-orders',auth,role(['customer']),rateLimit('group-orders',12,60*60*1000),(req,res)=>{const restaurantId=Number(req.body.restaurantId),restaurant=db.prepare("SELECT id,name FROM restaurants WHERE id=? AND active=1 AND operational_status NOT IN ('closed','paused')").get(restaurantId);if(!restaurant)return res.status(404).json({error:'Restaurante no disponible'});const code=crypto.randomBytes(5).toString('hex').toUpperCase(),expiresAt=new Date(Date.now()+24*60*60*1000).toISOString();const result=db.prepare("INSERT INTO group_orders(invite_code,creator_user_id,restaurant_id,expires_at) VALUES(?,?,?,?)").run(code,req.user.id,restaurantId,expiresAt);res.status(201).json({id:Number(result.lastInsertRowid),code,restaurantName:restaurant.name,expiresAt,url:'/group-order.html?code='+code});});
