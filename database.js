...
ensureColumn('order_reviews','food_rating','INTEGER');
ensureColumn('order_reviews','completeness_rating','INTEGER');
ensureColumn('order_reviews','preparation_rating','INTEGER');
ensureColumn('order_reviews','punctuality_rating','INTEGER');
ensureColumn('order_reviews','courtesy_rating','INTEGER');
ensureColumn('order_reviews','delivery_quality_rating','INTEGER');
...
CREATE TABLE IF NOT EXISTS driver_goals(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,period_type TEXT NOT NULL CHECK(period_type IN ('daily','weekly')),target_deliveries INTEGER NOT NULL CHECK(target_deliveries BETWEEN 1 AND 200),bonus_amount REAL NOT NULL CHECK(bonus_amount>=0 AND bonus_amount<=10000),starts_at TEXT NOT NULL,ends_at TEXT NOT NULL,active INTEGER NOT NULL DEFAULT 1,created_by_user_id INTEGER,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(created_by_user_id) REFERENCES users(id) ON DELETE SET NULL);
CREATE TABLE IF NOT EXISTS driver_goal_progress(id INTEGER PRIMARY KEY AUTOINCREMENT,goal_id INTEGER NOT NULL,delivery_user_id INTEGER NOT NULL,period_key TEXT NOT NULL,deliveries INTEGER NOT NULL DEFAULT 0,completed_at TEXT,updated_at TEXT DEFAULT CURRENT_TIMESTAMP,UNIQUE(goal_id,delivery_user_id,period_key),FOREIGN KEY(goal_id) REFERENCES driver_goals(id) ON DELETE CASCADE,FOREIGN KEY(delivery_user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS driver_rewards(id INTEGER PRIMARY KEY AUTOINCREMENT,delivery_user_id INTEGER NOT NULL,goal_id INTEGER,period_key TEXT,amount REAL NOT NULL CHECK(amount>=0),reward_type TEXT NOT NULL CHECK(reward_type IN ('goal_bonus','manual_bonus')),reason TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'earned' CHECK(status IN ('earned','paid','cancelled')),earned_at TEXT DEFAULT CURRENT_TIMESTAMP,paid_at TEXT,paid_by_user_id INTEGER,reference TEXT,UNIQUE(delivery_user_id,goal_id,period_key),FOREIGN KEY(delivery_user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(goal_id) REFERENCES driver_goals(id) ON DELETE SET NULL,FOREIGN KEY(paid_by_user_id) REFERENCES users(id) ON DELETE SET NULL);
CREATE INDEX IF NOT EXISTS idx_driver_rewards_status ON driver_rewards(status,earned_at);
CREATE TABLE IF NOT EXISTS driver_streaks(delivery_user_id INTEGER PRIMARY KEY,current_days INTEGER NOT NULL DEFAULT 0,best_days INTEGER NOT NULL DEFAULT 0,last_delivery_date TEXT,updated_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(delivery_user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS order_messages(id INTEGER PRIMARY KEY AUTOINCREMENT,order_id INTEGER NOT NULL,sender_user_id INTEGER NOT NULL,sender_role TEXT NOT NULL,channel TEXT NOT NULL CHECK(channel IN ('customer_restaurant','customer_delivery')),message TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,delete_after TEXT NOT NULL DEFAULT (datetime('now','+30 days')),FOREIGN KEY(order_id) REFERENCES orders(id) ON DELETE CASCADE,FOREIGN KEY(sender_user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE INDEX IF NOT EXISTS idx_order_messages_thread ON order_messages(order_id,channel,id);
CREATE INDEX IF NOT EXISTS idx_order_messages_retention ON order_messages(delete_after);
CREATE TABLE IF NOT EXISTS group_orders(id INTEGER PRIMARY KEY AUTOINCREMENT,invite_code TEXT NOT NULL UNIQUE,creator_user_id INTEGER NOT NULL,restaurant_id INTEGER NOT NULL,status TEXT NOT NULL DEFAULT 'open' CHECK(status IN ('open','completed','cancelled','expired')),expires_at TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,completed_at TEXT,FOREIGN KEY(creator_user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(restaurant_id) REFERENCES restaurants(id) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS restaurant_special_hours(id INTEGER PRIMARY KEY AUTOINCREMENT,restaurant_id INTEGER NOT NULL,service_date TEXT NOT NULL,is_closed INTEGER NOT NULL DEFAULT 1,opens_at TEXT,closes_at TEXT,note TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP,UNIQUE(restaurant_id,service_date),FOREIGN KEY(restaurant_id) REFERENCES restaurants(id) ON DELETE CASCADE);
CREATE INDEX IF NOT EXISTS idx_restaurant_special_hours_date ON restaurant_special_hours(restaurant_id,service_date);
