...
- Para recuperación por correo, verificar un remitente en SendGrid y definir en Render `SENDGRID_API_KEY`, `SENDGRID_FROM_EMAIL`, `SENDGRID_FROM_NAME` y `APP_BASE_URL=https://come-sayula.onrender.com`. La clave debe tener únicamente el permiso **Mail Send** y nunca debe guardarse en GitHub. Nunca activar `DEV_SHOW_RESET_TOKEN=1` en producción.
- El servidor envía `/reset-password.html?token=TOKEN` y mantiene una respuesta genérica para no revelar qué cuentas existen. El enlace vence en 30 minutos, sólo se usa una vez y al cambiar la contraseña se cierran las sesiones anteriores. Si SendGrid falla, el token que no fue entregado se invalida. Activar `ACCOUNT_MESSAGE_PROVIDER_ENABLED=1` únicamente después de probar un correo real de extremo a extremo.
...
- Mercado Pago Checkout Pro está integrado en modo de prueba. Mantener `MERCADOPAGO_MODE=test` y `PAYMENT_PROVIDER_ENABLED=0` hasta completar pruebas, configurar el webhook y recibir autorización expresa para cobros reales. La API rechaza campos con número de tarjeta, vencimiento o código de seguridad tanto en pedidos como en el punto de venta.
- Variables de prueba: `MERCADOPAGO_TEST_PUBLIC_KEY` y `MERCADOPAGO_TEST_ACCESS_TOKEN`. El servidor crea la preferencia y verifica el pago directamente; ninguna credencial ni dato de tarjeta se envía al navegador.
- Antes de producción, configurar en Mercado Pago el webhook `https://come-sayula.onrender.com/api/payments/mercadopago/webhook`, guardar el secreto como `MERCADOPAGO_WEBHOOK_SECRET`, cargar `MERCADOPAGO_ACCESS_TOKEN`, cambiar `MERCADOPAGO_MODE=production` y sólo entonces, con autorización, establecer `PAYMENT_PROVIDER_ENABLED=1`.
