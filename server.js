...
app.post('/api/orders',auth,role(['customer']),rateLimit('orders',12,10*60*1000),(req,res)=>{
    ...
    const getProduct=db.prepare('SELECT id,name,price,available,category FROM products WHERE id=? AND restaurant_id=?');
...
app.post('/api/restaurant/products',auth,restaurantAccess('can_manage_products'),(req,res)=>{
    ...
    if(image&&!image.startsWith('/uploads/'))return res.status(400).json({error:'Imagen inválida'});
    const result=db.prepare('INSERT INTO products(restaurant_id,name,description,price,image,category,available) VALUES(?,?,?,?,?,?,1)')
        .run(restaurant.id,name,description,price,image,category);
...
app.put('/api/restaurant/products/:id',auth,restaurantAccess('can_manage_products'),(req,res)=>{
    ...
    const result=db.prepare('UPDATE products SET name=?,description=?,price=?,image=?,category=? WHERE id=? AND restaurant_id=?')
        .run(name,description,price,image,category,req.params.id,restaurant.id);
