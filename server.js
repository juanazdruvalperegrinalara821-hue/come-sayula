...
app.post('/api/orders',auth,role(['customer']),rateLimit('orders',ORDER_RATE_LIMIT_MAX,10*60*1000),async(req,res,next)=>{try{
    ...
    const risk=assessOrderRisk(req.user.id,total,paymentMethod);
    if(paymentMethod==='Efectivo'&&total>risk.cashLimit)return res.status(409).json({error:'Por seguridad, tu límite actual para efectivo es de $'+risk.cashLimit.toFixed(2)+'. Elige transferencia, tarjeta al recibir o pago en línea.',riskLevel:risk.level,cashLimit:risk.cashLimit});
    if(total>=LARGE_ORDER_AMOUNT&&paymentMethod==='Efectivo')return res.status(409).json({error:'Los pedidos de monto alto requieren un método distinto de efectivo y confirmación adicional.',riskLevel:'verification'});
...
app.patch('/api/delivery/orders/:id',auth,role(['delivery']),(req,res)=>{
    ...
    if(nuevoEstado==='delivered'){
        ...
        let resultado;try{resultado=db.transaction(()=>{
            ...
            const trustActors=db.prepare('SELECT o.customer_id,r.owner_id FROM orders o JOIN restaurants r ON r.id=o.restaurant_id WHERE o.id=?').get(orderId);adjustTrust(trustActors.customer_id,1);adjustTrust(trustActors.owner_id,1);adjustTrust(req.user.id,1);
