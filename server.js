...
app.post('/api/orders',auth,role(['customer']),rateLimit('orders',ORDER_RATE_LIMIT_MAX,10*60*1000),async(req,res,next)=>{try{
    ...
    const restaurant=db.prepare("SELECT r.id,r.latitude,r.longitude,r.operational_status,r.prep_minutes FROM restaurants r JOIN users owner ON owner.id=r.owner_id AND owner.account_status='approved' WHERE r.id=? AND r.active=1").get(restaurantId);
    if(!restaurant)return res.status(404).json({error:'Restaurante no disponible'});
